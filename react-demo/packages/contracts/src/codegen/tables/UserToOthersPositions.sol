...
library UserToOthersPositions {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "", name: "UserToOthersPosi", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462000000000000000000000000000055736572546f4f7468657273506f7369);
   ...
   */
  function encodeLengths(bytes32[] memory snakes) internal pure returns (EncodedLengths _encodedLengths) {
    // Lengths are effectively checked during copy by 2**40 bytes exceeding gas limits
    unchecked {
      _encodedLengths = EncodedLengthsLib.pack(snakes.length * 32);
    }
  }
   ...
   */
  function encodeDynamic(bytes32[] memory snakes) internal pure returns (bytes memory) {
    return abi.encodePacked(EncodeArray.encode((snakes)));
  }
   ...
   */
  function encode(bytes32[] memory snakes) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData;
    EncodedLengths _encodedLengths = encodeLengths(snakes);
    bytes memory _dynamicData = encodeDynamic(snakes);
